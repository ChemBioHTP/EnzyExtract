...
def script0():
    # prepare nonbrenda 
    # "topoff/open", "wos/jbc", 
    sources = ["scratch/asm", "scratch/hindawi", "scratch/open", "scratch/wiley", "topoff/hindawi", "topoff/wiley", "wos/asm", "wos/hindawi", "wos/local_shim", "wos/open", "wos/wiley"]
    
    # filenames = [f"_valid_{src.replace('/', '-')}-apogee-t2neboth_1.csv" for src in sources]
    # valid_paths = [f"data/_cache_valid/{filename}" for filename in filenames]
    ...
    compile_jsonls(valid_paths, "nonbrenda")

def script1():
    sources = [filename for filename in os.listdir('data/_cache_vbrenda') if filename.endswith('.csv')]
    ...
    for s in sources:
        ...
        if s.endswith('-apogee-t2neboth_1.csv'):
            good.append(s)
            continue
    compile_csvs([f'data/_cache_vbrenda/{s}' for s in good], "brenda-apogee")
