...
def script1():
    sources = [filename for filename in os.listdir('data/matched/with_brenda') if filename.endswith('.csv')]
    ...
    for s in sources:
        ...
        if s.endswith('-apogee-t2neboth_1.csv'):
            good.append(s)
            continue
    compile_csvs([f'data/matched/with_brenda/{s}' for s in good], "apogee-brenda")
