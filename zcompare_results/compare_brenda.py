...
if __name__ == "__main__":
    ...
    runeem_df = pd.read_csv('data/humaneval/runeem/runeem_20241125_ec.csv', dtype={'pmid': str})
    ...
    whitelist = set(runeem_df['pmid'])

    # write_dest = f"data/humaneval/comparisons/compare_{namespace}_runeem.csv"
    write_dest = f"data/humaneval/comparisons/tupled/tupled_{namespace}_brenda.csv"
    ...
    view = df.select([
        'pmid', 'enzyme_or_ec_match', 'ec_2', 'viable_ecs', 'ec_match', 'enzyme', 'enzyme_full', 'enzyme_preferred', 'enzyme_2', 'enzyme_match', 'substrate', 'substrate_full', 'substrate_preferred', 'substrate_2', 'substrate_match'
    ])
    ...
    view.write_parquet(f'data/humaneval/comparisons/rich/view_{namespace}_brenda.parquet')
