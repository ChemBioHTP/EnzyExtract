...
def accept_filename(x: str):
    return x.endswith('.jsonl') and 'prod1' in x
    ...
    for filename in tqdm(os.listdir('completions/pick')):
        ...
        with open(filepath, 'r') as f:
            for line in f:
                ...
                collected.append((# cid, 
                                  idx, pmid, # content, 
                    thoughts, best, second, third, stop_reason))
...
print(df)
df.write_parquet('data/gpt/uniprot_prod1.parquet')
enzyme_thesaurus = df.select(['pmid', 'enzyme', 'enzyme_full', 'organism', 'best']).rename({'best': 'uniprot'})
