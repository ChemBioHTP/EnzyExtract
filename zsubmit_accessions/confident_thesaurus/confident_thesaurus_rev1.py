...
def get_wanted_names():
    ...
    infos = infos.with_columns([
        ...
        pl.when(
            pl.col('enzyme').str.contains(' ')
            & pl.col('enzyme').str.len_chars() > 20
        ).then(
            pl.coalesce(pl.col('enzyme_full'), pl.col('enzyme'))
        ).otherwise(pl.col('enzyme_full'))
            .alias('enzyme_long'), # populate enzyme_long if the enzyme is complicated and sufficiently long
...
def get_all_cited():
    ...
    f_pdb = f_pdb.explode('pdb').with_columns(
        ...
        # exclude PDB with lowercase and uppercase, which often includes false positives like "mM" or "mL"
...
def perform_fuzz(uall, pall, nall):
    ...
    PDB_comparisons = [
        ...
        ('enzyme', 'PDB_descriptor', False, 'similarity_enzyme_vs_descriptor'), # pdb is always caps case, cannot use cases
        ('enzyme', 'PDB_name', False, 'similarity_enzyme_vs_name'),
        ('enzyme', 'PDB_info', False, 'similarity_enzyme_vs_info'),
        ('enzyme_long', 'PDB_descriptor', False, 'similarity_enzymelong_vs_descriptor'),
        ('enzyme_long', 'PDB_name', False, 'similarity_enzymelong_vs_name'),
        ('enzyme_long', 'PDB_info', False, 'similarity_enzymelong_vs_info'),
        
    ]

    pneeded = pall.select([
        'organism', 'organism_scientific', 'enzyme', 'enzyme_long', 'pdb_versioned', 'PDB_organism', 'PDB_descriptor', 'PDB_name', 'PDB_info'
    ]).unique(['organism', 'enzyme', 'enzyme_long', 'pdb_versioned'])
    pneeded = compute_fuzz_with_progress(pneeded, PDB_comparisons).with_columns(
        pl.max_horizontal(
            ...
            pl.col(f"similarity_enzymelong_vs_descriptor"),
            pl.col(f"similarity_enzymelong_vs_name"),
            pl.col(f"similarity_enzymelong_vs_info")
        ...
        cs.starts_with('max_'),
        'organism', 'enzyme', 'enzyme_long', 'pdb_versioned'
    )
    ...
    pall = pall.join(pneeded, left_on=['organism', 'enzyme', 'enzyme_long', 'pdb_versioned'], right_on=['organism', 'enzyme', 'enzyme_long', 'pdb_versioned'], how='left')
    ...
    UNIPROT_comparisons = [
        ...
        ('enzyme', 'UNIPROT_enzyme_name', True, 'similarity_enzyme_vs_name'),
        ('enzyme', 'UNIPROT_recommended_name', True, 'similarity_enzyme_vs_recommended'),
        ('enzyme', 'UNIPROT_submission_names', True, 'similarity_enzyme_vs_submission'),
        ('enzyme', 'UNIPROT_alternative_names', True, 'similarity_enzyme_vs_alternative'),
        ('enzyme_long', 'UNIPROT_enzyme_name', False, 'similarity_enzymelong_vs_name'),
        ('enzyme_long', 'UNIPROT_recommended_name', False, 'similarity_enzymelong_vs_recommended'),
        ('enzyme_long', 'UNIPROT_submission_names', False, 'similarity_enzymelong_vs_submission'),
        ('enzyme_long', 'UNIPROT_alternative_names', False, 'similarity_enzymelong_vs_alternative'),
    ]

    uneeded = uall.select([
        'organism', 'organism_scientific', 'enzyme', 'enzyme_long', 'uniprot', 
    ...
    ]).unique(['organism', 'enzyme', 'enzyme_long', 'uniprot'])
    uneeded = compute_fuzz_with_progress(uneeded, UNIPROT_comparisons).with_columns([
        ...
        pl.max_horizontal(
            ...
            pl.col(f"similarity_enzymelong_vs_name"),
            pl.col(f"similarity_enzymelong_vs_recommended"),
            pl.col(f"similarity_enzymelong_vs_submission"),
            pl.col(f"similarity_enzymelong_vs_alternative"),
        ).alias('max_enzyme_similarity'),
    ]).select(
        cs.starts_with('similarity_'),
        cs.starts_with('max_'),
        'organism', 'enzyme', 'enzyme_long', 'uniprot'
    )
    uall = uall.join(uneeded, left_on=['organism', 'enzyme', 'enzyme_long', 'uniprot'], right_on=['organism', 'enzyme', 'enzyme_long', 'uniprot'], how='left')

    ### NCBI
    NCBI_comparisons = [
        ('enzyme', 'NCBI_descriptor', True, 'similarity_enzyme_common'),
        ('enzyme_long', 'NCBI_descriptor', False, 'similarity_enzymelong_common'),
    ...
    nneeded = nall.select([
        'organism', 'organism_scientific', 'enzyme', 'enzyme_long', 'ncbi', 'NCBI_descriptor'
    ]).unique(['organism', 'enzyme', 'enzyme_long', 'ncbi'])
    nneeded = compute_fuzz_with_progress(nneeded, NCBI_comparisons).with_columns(
        ...
        pl.max_horizontal(
            pl.col(f"similarity_enzyme_common"),
            pl.col(f"similarity_enzymelong_common")
        ).alias('max_enzyme_similarity'),
    ).select(
        cs.starts_with('similarity_'),
        cs.starts_with('max_'),
        'organism', 'enzyme', 'enzyme_long', 'ncbi'
    )
    nall = nall.join(nneeded, on=['organism', 'enzyme', 'enzyme_long', 'ncbi'], how='left')
