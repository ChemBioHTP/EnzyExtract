...
def extract_enzyme_accessions(df: pl.LazyFrame, col, keep=None):
    """
    df: should have columns 'pmid', 'page_number', and {col}

    Returns: df with columns 'pdb', 'uniprot', 'refseq', 'genbank', and 'has_pdb'

    Caveat: searching for PDB alone, there are many false positives. Hence, we also look for
    the words "Protein Data Bank" (or variations) anywhere in the main text.
...
@export("data/enzymes/sequence_scans/{alias}_sequences.parquet")
def script_look_for_protein_identifiers():
    # dfs = []

    in_df_names = [
        x.removesuffix('.parquet')
        for x in os.listdir('data/scans')
        if x.endswith('.parquet') and 'xml' not in x
    ]
...
@resolve
@export("data/enzymes/sequence_scans/xml_sequences.parquet")
def script_look_for_protein_identifiers_xml(
    df = REQUIRE('data/scans/xml_slim.parquet', eager=True)
):
    ...
    # df = pl.read_parquet('data/scans/xml_slim.parquet')
    df = df.select(['pmid', 'content'])


    with tqdm(total=df.height) as pbar:

        df = df.with_columns([
            pl.col('content').map_elements(wrap_pbar(pbar, _extract_accessions_from_text), return_dtype=pl.Struct({
                "pdb": pl.List(pl.Utf8),
...
@resolve
@export("data/enzymes/sequence_scans/xml_abstract_sequences.parquet")
def script_look_for_protein_identifiers_xml_abstract(
    df = REQUIRE('data/scans/xml_slim.parquet', eager=False)
):
    # df = pl.scan_parquet('data/scans/xml_slim.parquet')
...
if __name__ == "__main__":
    os.makedirs('data/enzymes/sequence_scans', exist_ok=True)
