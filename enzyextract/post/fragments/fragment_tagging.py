...
def extract_fragments(
    ...
    contextdf = contextdf.unique('custom_id', keep='last', maintain_order=True).with_row_index('context_id')
    ...
    # fragment_id unique but data_id not unique
    # weird = enzyme_exact_full.filter(
    #     enzyme_exact_full.select("data_id").is_duplicated()
    # )
    ...
    enzyme_coalesced, coalesced_values = coalesce_collect(
        ...
        column_renames=['enzyme_match', 'enzyme_id'],
        join_key='data_id',
        final_join_strategy="best",
    ) # 178471 - 177738 = 733
    ...
    pH_coalesced, _ = coalesce_collect(
        [
            (pH_exact, 'exact', ['fragments']),
            (pH_exact2, 'exact2', ['fragments']),
            (pH_inexact, 'inexact', ['extract']),
        ],
        column_renames=['pH_match'],
        join_key='data_id',
        final_join_strategy="best",
    )
    pH_coalesced = pH_coalesced.sort('data_id')

    temp_coalesced, _ = coalesce_collect(
        [
            (temp_exact, 'exact', ['fragments']),
            (temp_exact_k, 'exact_k', ['fragments']),
            (temp_inexact, 'inexact', ['extract']),
        ],
        column_renames=['temp_match'],
        join_key='data_id',
        final_join_strategy="best",
    )
    temp_coalesced = temp_coalesced.sort('data_id')

    cofactors_coalesced, _ = coalesce_collect(
        [
            (cofactors, 'cofactors', ['fragments']),
            (cofactors_inexact, 'cofactors_inexact', ['extract']),

            (cofactors_inexact_ends, 'cofactors_inexact_ends', ['fragments']),  
            # also has "shrinkable", but the point is that if it ends with " mM" --> likely a cofactor, take whole fragment
            #

            (cofactors_ions, 'cofactors_ions', ['fragments']),
        ],
        column_renames=['cofactor_match'],
        join_key='data_id',
        final_join_strategy="all",  # extract all (could have multiple cofactors)
    )
    cofactors_coalesced = cofactors_coalesced.sort('data_id')
    return {
        "descriptor": descriptors,  # pkey: data_id, others: descriptor: str, fragments: list[str], context_id, custom_id
        "fragments": fragments,  # pkey: fragment_id, others: descriptor: str, fragments: str
        # exact: fragment_id, data_id, custom_id, descriptor: str, fragments: str, context_id
        # inexact: fragment_id, data_id, custom_id, descriptor: str, fragments: str, context_id, extract: str
        "pH": pH_coalesced,
        "temperature": temp_coalesced,
        "cofactors": cofactors_coalesced,

        # pkey: data_id, xxx.coalesce_id, best_coalesce_id, enzyme_match: str (enzyme name), enzyme_id, descriptor (maps descriptor to enzyme name))
        "enzymes": enzyme_coalesced,

        # REMINDER: contextdf contains pkey: context_id, others: custom_id, enzymes: list[struct], substrates: list[struct], temperatures, pHs,
        "enzyme_context": enzymedf, # pkey: enzyme_id, others: context_id, fullname, synonyms, mutants, organisms
    }

if __name__ == '__main__':
    datadf = pl.read_parquet('data/recontext/1_fromyaml/data.parquet')
    contextdf = pl.read_parquet('data/recontext/1_fromyaml/context.parquet')
    extract_fragments(datadf, contextdf)