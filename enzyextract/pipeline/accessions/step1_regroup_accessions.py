...
def get_total_accessions():
    ...
    filenames = [
        ...
        for x in os.listdir('data/enzymes/sequence_scans')
        if x.endswith('_sequences.parquet')
    ]
...
def get_known_accessions():
    ...
    if (
        os.path.exists('data/enzymes/accessions/pdb')
        or len(os.listdir('data/enzymes/accessions/pdb'))
    ):
        ...
        # PDB accessions not found. This is OK!
        pdb_known = pl.DataFrame(schema=pdb_df_schema)
...
@export("data/enzymes/accessions/unknown/unknown_pdb.parquet")
@export("data/enzymes/accessions/unknown/unknown_uniprot.parquet")
@export("data/enzymes/accessions/unknown/unknown_refseq.parquet")
@export("data/enzymes/accessions/unknown/unknown_genbank.parquet")
def get_unknown_accessions(deduct_known=False):
    """
    ...
    if deduct_known:
        # If accessions are already known
        ...
        pdb_unknown = pdb.select('pdb_unversioned').rename({'pdb_unversioned': 'pdb'})
        uniprot_unknown = uniprot
