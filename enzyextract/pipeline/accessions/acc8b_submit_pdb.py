...
from enzyextract.submit.openai_management import submit_openai_batch_file
from enzyextract.submit.openai_schema import to_openai_batch_request_with_schema



def _clean_pdb_str(x):
    if x:
        x = x.replace("\n", "")
        if x.isupper():
            return x.lower()
...
def to_target(enzyme, enzyme_full, organism, df_of_accessions):
    ...
    for pdb, descriptor, name, sys_name, organism, info in \
        ...
        descriptor = _clean_pdb_str(descriptor)
        name = _clean_pdb_str(name)
        sys_name = _clean_pdb_str(sys_name)
        organism = _clean_pdb_str(organism)
        info = _clean_pdb_str(info)
...
def submit_pdb_pick_file(
    read_from: Union[str, Path],
...
if __name__ == "__main__":
    submit_pdb_pick_file(
        read_from="data/enzymes/pick/pick-pdb-v1.parquet",
        model_name="gpt-4o",
